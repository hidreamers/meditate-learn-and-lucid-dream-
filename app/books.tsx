...
// --- Your existing books and chapter data ---
const books = [
  {
    id: '1',
...
// --- Example: Convergence chapters (add images if you want them in the player) ---
...
// --- Example: Manifestation chapters (with images) ---
const manifestationChapters = [
  {
...
// --- Example: The Seventh Angel chapters (with images) ---
const seventhAngelChapters = [
  {
...
export default function BooksScreen() {
  ...
  // Helper to open modal with chapter info
  const openPlayer = (chapter, withImage = false) => {
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        {books.map(book => (
          <View key={book.id} style={styles.bookCard}>
            ...
            {/* Audio Book Pop-out Player Buttons */}
            {book.title === "Convergence and the Love Code" &&
              convergenceChapters.map((chapter, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={audioStyles.dropdownItem}
                  onPress={() => openPlayer(chapter, false)}
                >
                  <Text style={audioStyles.dropdownItemText}>{chapter.title}</Text>
                  <Text style={audioStyles.icon}>🎧</Text>
                </TouchableOpacity>
              ))}
            {book.title === "Manifestation Through Spiritual Power: Unlock the secrets of Lucid Dreaming and Alchemy" &&
              manifestationChapters.map((chapter, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={audioStyles.dropdownItem}
                  onPress={() => openPlayer(chapter, true)}
                >
            ...
            {book.title === "The Seventh Angel" &&
              seventhAngelChapters.map((chapter, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={audioStyles.dropdownItem}
                  onPress={() => openPlayer(chapter, true)}
            ...
            {book.title.startsWith("A Dreamer’s Odyssey") &&
              odysseyChapters.map((chapter, idx) => (
                <TouchableOpacity
                  key={idx}
                  style={audioStyles.dropdownItem}
                  onPress={() => openPlayer(chapter, false)}
